...
public final class GroupAdvertisementTopicTypeSupport implements TypeSupport{
  
  public ReturnCode_t register_type(DomainParticipant dp, String type_name) {
    if (dp.check_version( "3", "4", "0_p9" ) != 0) {
      System.out.println( "WARNING: GroupAdvertisementTopic TypeSupport version does not match CoreDX Library version.");
      System.out.println( "This may cause software instability or crashes.");
    }
    return dp.register_type(this, type_name);
  }

  public String get_type_name()   { return "GroupAdvertisementTopic"; }
  public long   getCTypeSupport() { return cTypeSupport; }

  public GroupAdvertisementTopicTypeSupport() {
    GroupAdvertisementTopic tmp = new GroupAdvertisementTopic();
  ...
  public DataReader   create_datareader(Subscriber sub, TopicDescription td, 
    ...
    return new GroupAdvertisementTopicDataReader(sub, td, jni_dr);
  }
  public DataWriter   create_datawriter(Publisher  pub, Topic topic, SWIGTYPE_p__DataWriter jni_dw ) {
    return new GroupAdvertisementTopicDataWriter(pub, topic, jni_dw);
  }

  // marshal variants
  public int     marshall ( ByteBuffer out_stream,  GroupAdvertisementTopic src ) {
    int size = 0;
    if ( out_stream == null ) {
      ...
      size += 4;
      // src.groupOperationCollection
      // src.groupOperationCollection.length
      size = (size+3) & 0xfffffffc;// align 4
      size += 4;
      if (src.groupOperationCollection != null) {
        for (int i6 = 0; i6 < src.groupOperationCollection.length; i6++ ) {
          // src.groupOperationCollection[i6]
          size = (size+3) & 0xfffffffc;// align 4
          size += 4;
        }
      }
      ...
      // src.groupOperationCollection.length
      while((out_stream.position() & 0x03) != 0) out_stream.put((byte)0x00); // align 4
      out_stream.putInt(src.groupOperationCollection.length);
      for (int i6 = 0; i6 < src.groupOperationCollection.length; i6++ ) {
          // src.groupOperationCollection[i6]
          while((out_stream.position() & 0x03) != 0) out_stream.put((byte)0x00); // align 4
          out_stream.putInt(src.groupOperationCollection[i6]);
      }
  ...
  public int     marshall_key ( ByteBuffer out_stream,  GroupAdvertisementTopic src ) {
    int size = 0;
    if ( out_stream == null ) {
      size += 4; // for CDR 'header' 
      // src.leastSignificantBitsVehicleId
  ...
  public int     marshall_key_hash ( ByteBuffer out_stream,  GroupAdvertisementTopic src ) {
    int size = 0;
    if ( out_stream == null ) {
      // src.leastSignificantBitsVehicleId
      size = (size+7) & 0xfffffff8;// align 8
  ...
  // unmarshal variants
  public int     unmarshall ( GroupAdvertisementTopic t, ByteBuffer data, int s )    {
    ...
    if ((encoding & 0x02)==0) { // CDR encoding
      ...
      // t.groupOperationCollection
      // t.groupOperationCollection.length
      while((data.position() & 0x03) != 0) data.position(data.position()+1); // align 4
      int    slen6 = data.getInt();
      t.groupOperationCollection = new int[slen6];
      for (int i6 = 0; i6 < t.groupOperationCollection.length; i6++ ) {
        // t.groupOperationCollection[i6]
        while((data.position() & 0x03) != 0) data.position(data.position()+1); // align 4
        t.groupOperationCollection[i6] = data.getInt();
      }
    }
    return 1; // 1==success, 0==failure
  }

  public int     unmarshall_key( GroupAdvertisementTopic t, ByteBuffer data, int s ) {
    data.get();                      // skip the first byte 
    byte encoding = data.get();      // data encoding CDR / ENDIAN 
    data.getShort();                 // unused flags (2 bytes)
    if ((encoding & 0x01)==0)  data.order(ByteOrder.BIG_ENDIAN);
  ...
  public int     unmarshall_key_hash( GroupAdvertisementTopic t, ByteBuffer data, int s ) {
  ...
  public int gen_typecode( ByteBuffer b ) {
    byte[] tc_data = { 
(byte)0x0a, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x38, (byte)0x01, (byte)0x00, (byte)0x00, (byte)0x18, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'G', (byte)'r', (byte)'o', (byte)'u', (byte)'p', (byte)'A', (byte)'d', (byte)'v', (byte)'e', (byte)'r', (byte)'t', (byte)'i', (byte)'s', (byte)'e', (byte)'m', (byte)'e', (byte)'n', (byte)'t', (byte)'T', (byte)'o', (byte)'p', (byte)'i', (byte)'c', (byte)0x00, (byte)0x06, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x30, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x1e, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'l', (byte)'e', (byte)'a', (byte)'s', (byte)'t', (byte)'S', (byte)'i', (byte)'g', (byte)'n', (byte)'i', (byte)'f', (byte)'i', (byte)'c', (byte)'a', (byte)'n', (byte)'t', (byte)'B', (byte)'i', (byte)'t', (byte)'s', (byte)'G', (byte)'a', (byte)'t', (byte)'e', (byte)'w', (byte)'a', (byte)'y', (byte)'I', (byte)'d', (byte)0x00, (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0x00, (byte)0x00, (byte)0x11, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x2e, (byte)0x00, (byte)0x1d, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'m', (byte)'o', (byte)'s', (byte)'t', (byte)'S', (byte)'i', (byte)'g', (byte)'n', (byte)'i', (byte)'f', (byte)'i', (byte)'c', (byte)'a', (byte)'n', (byte)'t', (byte)'B', (byte)'i', (byte)'t', (byte)'s', (byte)'G', (byte)'a', (byte)'t', (byte)'e', (byte)'w', (byte)'a', (byte)'y', (byte)'I', (byte)'d', (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x11, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x2e, (byte)0x00, (byte)0x1e, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'l', (byte)'e', (byte)'a', (byte)'s', (byte)'t', (byte)'S', (byte)'i', (byte)'g', (byte)'n', (byte)'i', (byte)'f', (byte)'i', (byte)'c', (byte)'a', (byte)'n', (byte)'t', (byte)'B', (byte)'i', (byte)'t', (byte)'s', (byte)'V', (byte)'e', (byte)'h', (byte)'i', (byte)'c', (byte)'l', (byte)'e', (byte)'I', (byte)'d', (byte)0x00, (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0x01, (byte)0x00, (byte)0x11, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x2e, (byte)0x00, (byte)0x1d, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'m', (byte)'o', (byte)'s', (byte)'t', (byte)'S', (byte)'i', (byte)'g', (byte)'n', (byte)'i', (byte)'f', (byte)'i', (byte)'c', (byte)'a', (byte)'n', (byte)'t', (byte)'B', (byte)'i', (byte)'t', (byte)'s', (byte)'V', (byte)'e', (byte)'h', (byte)'i', (byte)'c', (byte)'l', (byte)'e', (byte)'I', (byte)'d', (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0x01, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x11, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x1a, (byte)0x00, (byte)0x0a, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'g', (byte)'r', (byte)'o', (byte)'u', (byte)'p', (byte)'T', (byte)'y', (byte)'p', (byte)'e', (byte)0x00, (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0x00, (byte)0x00, (byte)0x02, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x36, (byte)0x00, (byte)0x19, (byte)0x00, (byte)0x00, (byte)0x00, (byte)'g', (byte)'r', (byte)'o', (byte)'u', (byte)'p', (byte)'O', (byte)'p', (byte)'e', (byte)'r', (byte)'a', (byte)'t', (byte)'i', (byte)'o', (byte)'n', (byte)'C', (byte)'o', (byte)'l', (byte)'l', (byte)'e', (byte)'c', (byte)'t', (byte)'i', (byte)'o', (byte)'n', (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x0e, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x0c, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0xff, (byte)0xff, (byte)0xff, (byte)0xff, (byte)0x02, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00,     };
  ...
  // <type> operations
  public GroupAdvertisementTopic         alloc ()              { return new GroupAdvertisementTopic(); }
  public void       clear (GroupAdvertisementTopic instance)   { instance.clear(); }
  public void       destroy (GroupAdvertisementTopic instance) { /* noop */ }
  public void       copy (GroupAdvertisementTopic to, GroupAdvertisementTopic from) { to.copy(from); }
