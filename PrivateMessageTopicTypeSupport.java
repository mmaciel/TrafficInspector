...
public final class PrivateMessageTopicTypeSupport implements TypeSupport{
  ...
  public int     marshall ( ByteBuffer out_stream,  PrivateMessageTopic src ) {
    ...
    if ( out_stream == null ) {
      ...
      size += 4;
      if (src.message != null) {
        for (int i6 = 0; i6 < src.message.length; i6++ ) {
          // src.message[i6]
          size += 1;
        }
      }
      ...
      for (int i6 = 0; i6 < src.message.length; i6++ ) {
          // src.message[i6]
          out_stream.put((byte)src.message[i6]);
      }
  ...
  public int     unmarshall ( PrivateMessageTopic t, ByteBuffer data, int s )    {
    ...
    if ((encoding & 0x02)==0) { // CDR encoding
      ...
      t.message = new byte[slen6];
      for (int i6 = 0; i6 < t.message.length; i6++ ) {
        // t.message[i6]
        t.message[i6] = data.get();
      }
    }
